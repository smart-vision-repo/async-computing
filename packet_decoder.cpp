...
PacketDecoder::PacketDecoder(std::string video_file_name)
    : video_file_name(video_file_name), vidIdx(-1), fmtCtx(nullptr), codec(nullptr), codecpar(nullptr), stopThreads(false) {
    if (!initialize()) {
...
void PacketDecoder::decode(const std::vector<AVPacket*>& pkts, int interval, int gopId, DecodeCallback callback) {
    {
        std::lock_guard<std::mutex> lock(queueMutex);
