...
std::vector<uint16_t> preprocess_image(const cv::Mat &img, int input_w,
  ...
  for (size_t i = 0; i < chw_img_data.size(); ++i) {
    ...
    unsigned short sign =
        (unsigned short)((int_val >> 16) & 0x8000); // sign from bit 31
    short exponent = (short)(((int_val >> 23) & 0xFF) - 127); // exponent
    unsigned int mantissa = int_val & 0x7FFFFF;               // mantissa

    unsigned short half_val;

    if (exponent == 128) { // NaN or Inf
      half_val = sign | 0x7C00 |
                 (mantissa ? 0x200 : 0); // 0x7C00 is inf, if mantissa then NaN
    } else if (exponent > 15) {          // Overflow
      half_val = sign | 0x7C00;          // Becomes Inf
    } else if (exponent <= -15) {        // Underflow or zero
      if (exponent < -24) {              // Absolute zero
        half_val = sign | 0x0000;
      } else { // Subnormal
        mantissa =
            (mantissa | 0x800000) >> (14 - exponent); // exponent is negative
        half_val = sign | (unsigned short)mantissa;
      }
    } else { // Normal value
      half_val = sign | ((exponent + 15) << 10) | (mantissa >> 13);
    }
  ...
  int main(int argc, char **argv) {
    ...
    // Output format: [batch, 4+num_classes, 8400] e.g. [1, 84, 8400] for COCO
    // (80 classes)
    ...
    // 1x3x320x320 / 1x3x640x640 / 1x3x1088x1920 We are preprocessing to
    // 640x640, so we set this dimension.
    ...
    // context->getBindingDimensions(input_idx); std::cout << "INFO: Actual
    // input dimensions set: "; for (int i = 0; i < actual_input_dims.nbDims;
    // ++i) std::cout << actual_input_dims.d[i] << " "; std::cout << std::endl;
    ...
    for (int i = 0; i < num_dets_yolo; ++i) {
      ...
      if (max_class_score > CONF_THRESHOLD) {
        if (current_class_id == target_class_id) { // Filter for "dog"
          ...
          // Convert to x_min, y_min, x_max, y_max for model input scale
          // (640x640)
    ...
    std::cout << "Detected '" << TARGET_CLASS_NAME
              << "' BBOXes (x_min, y_min, x_max, y_max) in original image "
                 "coordinates:"
              << std::endl;
    for (const auto &det : nms_results) {
      // Scale bbox back to original image dimensions
      cv::Rect box_model_scale =
          det.bbox; // This is on the 640x640 padded image
